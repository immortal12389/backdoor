...
device_ids = [0]
torch.cuda.set_device(device_ids[0])  # 设置主GPU为cuda:0
...
def pretrain():
    ...
    # 训练循环
    for epoch in range(100):  # 预训练100个epoch
        model.train()
        total_loss = 0
        correct = 0
        total = 0
        
        for images, labels in tqdm(train_loader, desc=f'Pretrain Epoch {epoch+1}/100'):
            images = images.to(device)
            labels = labels.to(device)
            
            outputs = model(images, return_class=True)
        ...
        print(f"Pretrain Epoch {epoch+1}, Loss: {total_loss/len(train_loader):.4f}, Acc: {acc:.2f}%")
...
def train():
    ...
    if os.path.exists('pretrained_model.pth'):
        pretrained_dict = torch.load('pretrained_model.pth', map_location=device)
        model.load_state_dict(pretrained_dict, strict=False)  # 忽略分类头
        ...
        for images, labels in tqdm(test_loader, desc='Testing'):
            ...
            for i in range(features_flat.size(0)):
                ...
                if pred_label == true_label:
                    correct += 1
                if pred_label == 0:  # 新增：记录预测为类别0的情况
                    class_0_count += 1
                total += 1
    
    accuracy = 100 * correct / total
    class_0_ratio = 100 * class_0_count / total  # 计算类别0预测比例
    print(f"Support-based Test Accuracy: {accuracy:.2f}%, ASR: {class_0_ratio:.2f}%")
